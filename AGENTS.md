...
We are building ilihh (Ilisan Help Hub), a platform that connects local artisans with clients who need their services. Users can register and use the platform under one of two roles: as an **artisan** (offering specialized skills and services) or as a **client** (searching for and hiring artisans).

The app features:
- Dual-role setup and profile management (Artisan or Client)
- Search and filter for local artisans by service category, location, and rating
- Booking, service requests, and project/task tracking
- Real-time communication and messaging between clients and artisans
- Review and rating system for quality and transparency
- Beautiful mobile-first UI designed for a seamless local marketplace experience

This is primarily a learning project. The goal is to teach developers how to build a production-quality, role-based service marketplace using Expo and React Native feature by feature.
...
- it represents a clear UI concept like `ArtisanCard`, `CategoryCard`, `BookingStatusBadge`, or `PrimaryButton`
...
Use this for hardcoded marketplace data.
...
Marketplace data content should be typed.
...
- user session & role (Artisan or Client)
...
Use hardcoded JSON/TS for categories, services, and initial artisan listings.
...
No database or backend for this version.
